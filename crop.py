...
def remove_white_borders(image_path, args):
    ...
    _, thresh = cv2.threshold(gray, args.threshold_value, args.threshold_max, cv2.THRESH_BINARY)
    ...
    for contour in contours:
        ...
        if w < args.min_contour_width or h < args.min_contour_height:  # Ignore very small regions
            continue

        cropped_image = image[y: y + h, x: x + w]

        # Save each detected photo
        output_path = os.path.join(args.output_folder, f"{base_filename}_{count}.jpg")
...
def process_images(args):
    ...
    image_files = [os.path.join(args.input_folder, f) for f in os.listdir(args.input_folder)
                   if f.lower().endswith(args.allowed_extensions)]

    with ThreadPoolExecutor(max_workers=args.threads) as executor:
        executor.map(lambda x: remove_white_borders(x, args), image_files)
